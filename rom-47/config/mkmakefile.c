...
makefile()
{
	...
	fprintf(ofp, "IDENT=-D%s", mk_raise(ident));
	if (profiling)
		fprintf(ofp, " -DGPROF");
	if (cputype == 0) {
...
next:
	...
	if (wd == (char *)EOF) {
		...
		if (first == 2) {
			(void) sprintf(fname, "files.%s", mk_raise(ident));
			first++;
			fp = fopen(fname, "r");
			if (fp != 0)
				goto next;
...
for (ftp = ftab; ftp != 0; ftp = ftp->f_next) {
	...
	*cp = '\0';
	...
	if (och == 's') {
		...
		fprintf(f, "\t${CC} -E ${COPTS} %sc >%sp\n", tp, tp);
		fprintf(f, "\t${AS} ${AFLAGS} %sp -o %so\n", tp, tp);
		fprintf(f, "\trm -f %sp %sc\n\n", tp, tp);
		continue;
	}
	...
	case NORMAL:
		...
		case MACHINE_NEXT:
			fprintf(f, "\t${CC} -S ${CFLAGS} ../%sc\n", np);
#ifdef	notdef
			fprintf(f, "\tmv %ss %sp\n", tp, tp);
			fprintf(f, "\t${C2} %sp %ss\n", tp, tp);
#endif	notdef
			fprintf(f, "\t${AS} ${AFLAGS} %ss -o %so\n", tp, tp);
			fprintf(f, "\trm -f %sp %ss\n", tp, tp);
			break;
		}
		break;
...
do_systemspec(f, fl, first)
	...
	int first;
{
	fprintf(f, "%s: .depend_done Makefile assym.h ${OBJS}\n", fl->f_needs);
	fprintf(f, "\t@echo loading %s\n\t@rm -f %s\n",
	    fl->f_needs, fl->f_needs);
	switch (machine) {

	case MACHINE_NEXT:
		fprintf(f, "\t${LD} -o %s -segaddr __TEXT ${RELOC} ${OBJS} ${LFLAGS}\n",
			fl->f_needs);
		break;
	}
	fprintf(f, "\t@size %s\n", fl->f_needs);
